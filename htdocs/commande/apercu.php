...
if ($id > 0 || ! empty($ref))
{
	...
	if ($object->fetch($id,$ref) > 0)
	{
		...
		if (file_exists($file))
		{
			...
			print '<td><a data-ajax="false"';
            if (!empty($conf->global->MAIN_DISABLE_FORCE_SAVEAS)) print ' target="blank"';
            print ' href="'.DOL_URL_ROOT . '/document.php?modulepart=commande&file='.urlencode($relativepath).'">'.$object->ref.'.pdf</a></td>';
			...
			if (file_exists($filedetail))
			{
				...
				print '<td><a data-ajax="false"';
				if (!empty($conf->global->MAIN_DISABLE_FORCE_SAVEAS)) print ' target="blank"';
				print ' href="'.DOL_URL_ROOT . '/document.php?modulepart=commande&file='.urlencode($relativepathdetail).'">'.$object->ref.'-detail.pdf</a></td>';
