...
if ($id > 0 || ! empty($ref))
{
    ...
    if ($object->fetch($id,$ref) > 0)
    {
        ...
        if (file_exists($file))
        {
            ...
            print ' href="'.DOL_URL_ROOT . '/document.php?modulepart=facture&file='.urlencode($relativepath).'">'.$object->ref.'.pdf</a></td>';
            ...
            if (file_exists($filedetail)) // facture detaillee supplementaire
            {
                ...
                print '<td><a';
                if (!empty($conf->global->MAIN_DISABLE_FORCE_SAVEAS)) print ' target="blank"';
                print ' href="'.DOL_URL_ROOT . '/document.php?modulepart=facture&file='.urlencode($relativepathdetail).'">'.$object->ref.'-detail.pdf</a></td>';
